		...
		<table width="100%" class="table table-insert" border="1"
			...
			<tr>
				...
				<td><select data-value="${ssm:Info_html(mmm.xingbie)}"
					...
					name="xingbie" class="class_xingbie3"><option value="男">男</option>
						<option value="女">女</option>
				</select> <script>
					$(".class_xingbie3").val(
							$(".class_xingbie3").attr("data-value"))
				</script> <span
					style="color: red;">*</span></td>
			</tr>
			<tr>
				...
				<td><select data-value="${ssm:Info_html(mmm.suodaibanji)}"
					...
					id="suodaibanji" name="suodaibanji" class="class_suodaibanji4"><ssm:sql
							var="select" type="select">SELECT * FROM banjixinxi ORDER BY id desc</ssm:sql>
						<c:forEach items="${select}" var="m">
							<option value="${m.banji}">${m.banji}</option>
						</c:forEach>
				</select> <script>
					$(".class_suodaibanji4").val(
							$(".class_suodaibanji4").attr("data-value"))
				</script>
