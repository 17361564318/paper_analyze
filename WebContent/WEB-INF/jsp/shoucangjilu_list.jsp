...
<body>
	...
	<table width="100%" class="table table-hover table-border">
		...
		<tbody>
			...
			<c:forEach items="${list}" var="map">
				<tr>
					<td width="25">${map.id}</td>
					<td><c:set var="ms" value="${map}" scope="page" /> <%
 ...
 %></td>
					<td width="135" align="center">${map.addtime}</td>
					<td width="75" align="center"><a
						href="shoucangjilu_delete.do?id=${map.id}"
						onclick="return confirm('真的要删除？')">删除</a> <a
						href="${map.biao}detail.do?id=${map.xwid}" target="_blank">详细</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
