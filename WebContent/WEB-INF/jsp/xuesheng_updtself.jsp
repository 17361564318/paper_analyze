		...
		<table width="100%" class="table table-insert" border="1"
			...
			<tr>
				...
				<td><select data-value="${ssm:Info_html(mmm.xingbie)}"
					...
					name="xingbie" class="class_xingbie10"><option value="男">男</option>
						<option value="女">女</option>
				</select> <script>
					$(".class_xingbie10").val(
							$(".class_xingbie10").attr("data-value"))
				</script> <span
					style="color: red;">*</span></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><select data-value="${ssm:Info_html(mmm.suozaibanji)}"
					...
					id="suozaibanji" name="suozaibanji" class="class_suozaibanji11"><ssm:sql
							var="select" type="select">SELECT * FROM banjixinxi ORDER BY id desc</ssm:sql>
						<c:forEach items="${select}" var="m">
							<option value="${m.banji}">${m.banji}</option>
						</c:forEach>
				</select> <script>
					$(".class_suozaibanji11").val(
							$(".class_suozaibanji11").attr("data-value"))
				</script>
			...
			<tr>
				<td><input name="id" value="${mmm.id}" type="hidden" /> <input
					name="referer" value="?" type="hidden" /> <input name="updtself"
					value="${updtself}" type="hidden" /> &nbsp;</td>
