...
public class Builder {
	...
	public String buildSelect(Query query) {
		...
		return sql.replace("%DISTINCT%", parseDistinct(query)).replace("%FIELD%", parseField(query))
				.replace("%TABLE%", parseTable(query)).replace("%FORCE%", parseForce(query))
				.replace("%JOIN%", parseJoin(query)).replace("%WHERE%", parseWhere(query))
				.replace("%HAVING%", parseHaving(query)).replace("%GROUP%", parseGroup(query))
				.replace("%ORDER%", parseOrder(query)).replace("%LIMIT%", parseLimit(query))
	...
	public String buildInsert(Query query, boolean replace) {
		...
		return insertSql.replace("%INSERT%", replace ? "REPLACE" : "INSERT").replace("%TABLE%", parseTable(query))
				.replace("%FIELD%", StringUtil.join(" , ", fields)).replace("%DATA%", StringUtil.join(" , ", values));
	}
	...
	public String buildDelete(Query query) {
		...
		return deleteSql.replace("%TABLE%", parseTable(query)).replace("%JOIN%", parseJoin(query))
				.replace("%WHERE%", parseWhere(query)).replace("%ORDER%", parseOrder(query))
				.replace("%LIMIT%", parseLimit(query)).replace("%LOCK%", parseLock(query));
	}
	...
	public String buildUpdate(Query query) {
		...
		// protected String updateSql = "UPDATE %TABLE% SET
		// %SET%%JOIN%%WHERE%%ORDER%%LIMIT% %LOCK%";
		return updateSql.replace("%TABLE%", parseTable(query)).replace("%SET%", StringUtil.join(" , ", set))
				.replace("%JOIN%", parseJoin(query)).replace("%WHERE%", parseWhere(query))
				.replace("%LOCK%", parseLock(query));
	}
